/**
 * An asset type
 *
 * * `ASSETS` - Assets
 * * `CAPITAL_RESERVERS_AND_LIABILITIES` - Capital, reserves and liabilities
 * * `CASH_AT_BANK_AND_IN_HAND` - Cash at bank and in hand
 * * `CURRENT_ASSETS` - Current assets
 * * `CURRENT_DEBTORS` - Debtors
 * * `CURRENT_INVESTMENTS` - Investments
 * * `CURRENT_STOCKS` - Stocks
 * * `INTANGLIBLE_ASSETS` - Intanglible assets
 * * `INVESTMENTS` - Investments
 * * `NON_CURRENT_ASSETS` - Non-current assets
 * * `TANGLIBLE_ASSETS` - Intanglible assets
 */
export declare type AssetCode = 'ASSETS' | 'CAPITAL_RESERVERS_AND_LIABILITIES' | 'CASH_AT_BANK_AND_IN_HAND' | 'CURRENT_ASSETS' | 'CURRENT_DEBTORS' | 'CURRENT_INVESTMENTS' | 'CURRENT_STOCKS' | 'INTANGLIBLE_ASSETS' | 'INVESTMENTS' | 'NON_CURRENT_ASSETS' | 'TANGLIBLE_ASSETS';
